...
assert.match(html, /class="report-section ai-probe-section genealogy-ai-probe"/);
assert.match(html, /class="section-heading ai-probe-heading"/);
assert.match(html, /class="ai-privacy-note"/);
assert.match(html, /class="ai-prompt-label" for="genealogyAIProbePrompt"/);
assert.match(html, /id="genealogyAIProbePrompt" rows="18" readonly spellcheck="false"/);
assert.match(html, /class="ai-prompt-actions"/);
assert.match(html, /class="result-actions"/);
assert.doesNotMatch(html, /class="results-actions"/);
assert.match(html, /Nothing is sent by this site/i);
assert.ok((css.match(/#genealogyAIProbePrompt/g) || []).length >= 2, "the genealogy prompt needs desktop and mobile sizing");
assert.match(css, /\.genealogy-conclusion\s*\{[^}]*color:\s*var\(--ink\)/s);
assert.match(css, /\.genealogy-conclusion > p\s*\{[^}]*color:\s*#465047/s);
